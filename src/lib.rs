...
impl Bme280 {
    ...
    fn temperature_correction_factor(&mut self) -> Result<f64, LinuxI2CError> {
    ...
    pub fn read_temperature(&mut self) -> Result<f64, LinuxI2CError> {
        ...
        let celcius = try!(self.temperature_correction_factor()) / 5120.0;
    ...
    pub fn read_pressure(&mut self) -> Result<f64, LinuxI2CError> {
        ...
        let t_fine = try!(self.temperature_correction_factor());
...
fn load_calibration(dev: &mut LinuxI2CDevice) -> Result<calibration::Calibration, LinuxI2CError> {
...
pub fn create(i2c_addr: u16, busnum: u8) -> Result<Bme280, LinuxI2CError> {
    ...
    let max_over_sampling_and_normal_mode = 0x3F;
    try!(device.smbus_write_byte_data(BME280_REGISTER_CONTROL, max_over_sampling_and_normal_mode));
    let bme280 = Bme280 { device: device, mode: BME280OSAMPLE1, calibration: calibration };
    Ok(bme280)
}

pub struct Bme280_2<T: I2CDevice + Sized> {
    i2cdev: T,
    calibration: calibration::Calibration
}

impl<T> Bme280_2<T> where T: I2CDevice + Sized {
    pub fn new(mut dev: T) -> Result<Bme280_2<T>, T::Error> {
